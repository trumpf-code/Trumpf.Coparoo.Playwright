...
{
    private static Lazy<Dictionary<Type, Type[]>> parentToChildrenMap = new(() => ParentToChildMap);
    ...
    /// </summary>
    internal static Type[] PageObjectTypes => pageObjectTypes ??= UIObjectTypes(pageObjectSelector);

    /// <summary>
    /// Gets the control object types in the current app domain.
    /// </summary>
    internal static Type[] ControlObjectTypes => controlObjectTypes ??= UIObjectTypes(controlObjectSelector);
