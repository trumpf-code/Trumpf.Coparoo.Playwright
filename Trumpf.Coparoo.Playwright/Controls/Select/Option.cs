...
public class Option : ControlObject, IOption
{
    ...
    public async Task Select()
    {
        if (!await IsSelected())
        {
            string value = await Node.GetAttributeAsync("value");
            await Parent.Node.SelectOptionAsync(value);
        }
    }
}